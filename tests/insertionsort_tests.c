...
char *test_int(){
	...
	insertion_sort(a, size_a, sizeof(int), compare_int);
	for(i = 1; i < size_a; i++)
	{
...
char *test_char(){
	...
	insertion_sort(cha, size_a, 20, compare_string);
	for(i = 1; i < size_a; i++){
...
char *test_char2(){
	...
	insertion_sort(cha2, size_a, sizeof(char *), compare_stringv2);
	for(i = 1; i < size_a; i++){
		mu_assert(!(strcmp(cha2[i], cha2[i-1]) < 0), "Not Sorted Char2");			
	}
